...
def find_network_string(string)
    if get_network_string('ipv4', @@ipv4_regex)
      string.scan(@@ipv4_regex)
    elsif get_network_string('ipv6', @@ipv6_regex)
      string.scan(@@ipv6_regex)
    elsif get_network_string('mac', @@mac_regex)
  ...
  def get_network_string(selection, search_string)
	case selection
		when 'mac' && unless search_string.scan(@@mac_regex).nil?
			true
		when 'ipv4' && unless search_string.scan(@@ipv4_regex).nil?
			true
		when 'ipv6' && unless search_string.scan(@@ipv6_regex).nil?
			true
		else
			false
		end
