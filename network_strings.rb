...
class Network_String
	def get_network_string(selection, search_string)
		if selection == "mac" and search_string.scan mac_regex!.eql? "" 
			true
		elif selection == "ipv4" and search_string.scan ipv4_regex!.eql?""
			true
		elif selection == "ipv6" and search_string.scan ipv6_regex!.eql?""
			true
		else
			false
		end
	...
	def find_network_string(string)
		if get_network_string("ipv4")
			string.scan ipv4_regex
		elsif get_network_string("ipv6")
			string.scan ipv6_regex
		elsif get_network_string("mac")
			string.scan mac_regex
		else
			""
		end
	end
